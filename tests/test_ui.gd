...
func move_to(id: String) -> void:
	...
	expect(app.game.data.location == id, "Travel reaches destination " + id)
	await settle()
...
func fight() -> void:
	expect(app.game.in_combat() and app.overlay_mode.is_empty(), "Battle starts automatically on arrival")
...
func _run() -> void:
	...
	expect("lookout" in app.game.data.resolved, "Healing resolves automatically on arrival")
	await close_story()
	await move_to("grove")
	expect(not app.game.in_combat() and app.overlay_mode.is_empty(), "Completed battle does not restart on return")
	app.game.get_hero("ivar").hp -= 1
	var hp_before_return: int = app.game.get_hero("ivar").hp
	await move_to("lookout")
	expect(app.overlay_mode.is_empty() and app.game.get_hero("ivar").hp == hp_before_return, "Completed healing does not repeat on return")
	await move_to("bridge")
	await fight()
	expect(app.game.data.level == 2, "UI campaign reaches level 2")
	await move_to("haven")
	expect(app.overlay_mode == "location", "Arrival opens camp without automatic rest")
	expect(app.game.get_hero("mira").uses < 3, "Arrival at camp does not restore spent charges")
	await press("Отдохнуть")
	await close_story()
	await move_to("sentinel")
	await fight()
	await move_to("chapel")
	expect("relic" in app.game.data.flags, "Relic is collected automatically on arrival")
	await close_story()
	await move_to("sentinel")
	await move_to("haven")
	await press("Отдохнуть")
	...
	# A fresh isolated route checks that arrival opens a choice without choosing for the player.
	app.game.reset(false)
	app.selected = "camp"
	app.build_ui()
	await settle()
	await move_to("fork")
	expect(app.overlay_mode.is_empty(), "Road without event stays on map")
	await move_to("ford")
	await move_to("hermit")
	expect(app.overlay_mode == "location", "Traveler choice opens automatically")
	expect(app.game.get_hero("vesta").uses == 3 and app.game.data.flags.is_empty(), "Arrival does not spend charges or choose a story branch")
	await press("Помочь")
	expect(app.game.get_hero("vesta").uses == 2 and "helped_traveler" in app.game.data.flags, "Player still controls the event choice")
	await close_story()
	await move_to("ford")
	await move_to("hermit")
	expect(app.overlay_mode.is_empty() and app.game.get_hero("vesta").uses == 2, "Resolved choice does not repeat on return")
	for button in buttons(app.root_layout):
